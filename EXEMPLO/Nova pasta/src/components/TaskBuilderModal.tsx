...
export default function TaskBuilderModal({ 
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const savedDraft = storage.get<Partial<TaskData>>(`task_builder_draft_${objectiveTitle}`, {});
      if (Object.keys(savedDraft).length > 0) {
        // If we have an initialMetaId, it should override the draft's metaId
        // if the draft's metaId is different (meaning we switched context)
        setFormData(prev => ({ 
          ...prev, 
          ...savedDraft, 
          id: 'draft-task',
          metaId: initialMetaId || savedDraft.metaId || prev.metaId
        }));
      } else {
        // Full reset if no draft exists
        setFormData({
          id: 'draft-task',
          title: '',
          description: '',
  ...
  useEffect(() => {
    if (isOpen) {
      const timer = setTimeout(() => {
        storage.set(`task_builder_draft_${objectiveTitle}`, formData);
        setLastSaved(new Date().toLocaleTimeString());
      }, 1500);
      return () => clearTimeout(timer);
    }
  }, [formData, isOpen, objectiveTitle]);
  ...
  const handleFinalize = () => {
    ...
    setIsSaving(true);
    setTimeout(() => {
      const finalTask = { ...formData, id: crypto.randomUUID() };
      onSave(finalTask);
      storage.remove(`task_builder_draft_${objectiveTitle}`);
      setIsSaving(false);
      onClose();
    }, 1000);
  };
    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          className="relative w-full h-full md:h-auto md:max-h-[95vh] max-w-full md:max-w-[95%] bg-neutral-black border-x-0 md:border border-neutral-white/10 rounded-none md:rounded-[3rem] overflow-y-auto md:overflow-hidden custom-scrollbar shadow-[0_0_100px_rgba(0,0,0,0.5)] flex flex-col"
        >
          {/* Header */}
          <header className="px-6 md:px-10 py-5 md:py-8 border-b border-neutral-white/5 flex items-center justify-between bg-neutral-black/40 backdrop-blur-xl relative md:sticky top-0 z-30 flex-shrink-0">
          ...
          <div className="flex-1 flex flex-col md:flex-row md:overflow-y-auto custom-scrollbar">
            ...
            {/* Footer Actions */}
            <footer className="px-6 md:px-10 pt-0 pb-1.5 md:py-8 border-t border-neutral-white/5 bg-neutral-black/80 backdrop-blur-xl flex flex-col sm:flex-row items-center justify-between gap-0 sm:gap-4 md:sticky bottom-0 z-30 flex-shrink-0">
              <button 
                onClick={onClose}
