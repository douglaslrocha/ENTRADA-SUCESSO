...
const sql = `
-- Create missing tables from 001_init.sql translated to PostgreSQL
...
-- Recreate objectives, metas, tarefas in Portuguese from 001_atacar_objetivos.sql
...
async function run() {
  ...
  try {
    ...
    console.log('Dropping table columns / old objectives tables to avoid mismatch...');
    await client.query('DROP TABLE IF EXISTS tasks CASCADE;');
    await client.query('DROP TABLE IF EXISTS goals CASCADE;');
    await client.query('DROP TABLE IF EXISTS objectives CASCADE;');
