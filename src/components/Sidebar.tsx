...
interface SidebarProps {
  ...
  onToggleTheme: () => void;
  theme: 'light' | 'dark';
  onNavigate: (page: 'editor' | 'manager' | 'dashboard' | 'cortes' | 'mural' | 'central' | 'ai' | 'projects' | 'diary' | 'central-de-comando' | 'amparadora' | 'profile' | 'presences' | 'amparo' | 'database-map') => void;
  currentPage: 'editor' | 'manager' | 'dashboard' | 'cortes' | 'mural' | 'central' | 'ai' | 'projects' | 'diary' | 'central-de-comando' | 'amparadora' | 'profile' | 'presences' | 'amparo' | 'database-map';
}
...
export default function Sidebar({ isOpen, onClose, onToggleTheme, theme, onNavigate, currentPage }: SidebarProps) {
  ...
  const mainNavItems = [
    ...
    { id: 'database-map', title: 'Mapa do Banco', desc: 'Estrutura e fluxo de dados', icon: 'database', color: '#c8e6c9' }, // Soft Green
  ];
                  ...
                  {mainNavItems.map((item) => (
                    ...
                    >
                      <div className="nav-card-icon">
                        ...
                        {item.id === 'database-map' && <span className="material-symbols-outlined text-2xl animate-pulse" style={{ color: item.color }}>database</span>}
                      </div>
