...
import { 
  ...
  ChevronLeft, Edit2, ChevronDown, Sliders
} from 'lucide-react';
...
export default function MultimodalExecutionModal({ 
  ...
  const [customFatuistica, setCustomFatuistica] = useState<string[]>(() => storage.get('energy_work_fatuistica', []));
  ...
  // CRUD handlers matching AmparoPage with total parity
  const handleAddSensation = () => {
    const name = prompt('Nova sinalética detectada:');
    if (name && name.trim()) {
      const trimmed = name.trim();
      if (!DEFAULT_SENSATIONS.includes(trimmed) && !customSensations.includes(trimmed)) {
  ...
  const handleEditSensation = (oldValue: string) => {
    const newVal = prompt('Editar sinalética:', oldValue);
    if (newVal && newVal.trim() && newVal.trim() !== oldValue) {
  ...
  const handleDeleteSensation = (value: string) => {
    if (confirm(`Excluir a sinalética customizada "${value}"?`)) {
  ...
  const handleAddPhenomenon = () => {
    ...
    if (name && name.trim()) {
      const trimmed = name.trim();
      if (!DEFAULT_PHENOMENA.includes(trimmed) && !customPhenomena.includes(trimmed)) {
        const updated = [...customPhenomena, trimmed];
  ...
  const handleEditPhenomenon = (oldValue: string) => {
    const newVal = prompt('Editar fenômeno:', oldValue);
    if (newVal && newVal.trim() && newVal.trim() !== oldValue) {
      const trimmed = newVal.trim();
  ...
  const handleDeletePhenomenon = (value: string) => {
    if (confirm(`Excluir o fenômeno customizado "${value}"?`)) {
  ...
  const handleAddFatuistica = () => {
    const name = prompt('Nova técnica/fenômeno (Fatuística):');
    if (name && name.trim()) {
      const trimmed = name.trim();
      if (!DEFAULT_FATUISTICA.some(o => o.label.toLowerCase() === trimmed.toLowerCase()) && !customFatuistica.includes(trimmed)) {
        const updated = [...customFatuistica, trimmed];
        setCustomFatuistica(updated);
        storage.set('energy_work_fatuistica', updated);
      }
    }
  };

  const handleEditFatuistica = (oldValue: string) => {
    const newVal = prompt('Editar fatuística:', oldValue);
    if (newVal && newVal.trim() && newVal.trim() !== oldValue) {
      const trimmed = newVal.trim();
  ...
  const handleDeleteFatuistica = (value: string) => {
    if (confirm(`Excluir a fatuística customizada "${value}"?`)) {
                ...
                <div className="p-8 bg-neutral-white/[0.02] border border-neutral-white/5 rounded-[4rem] flex flex-col space-y-12">
                  ...
                  <div className="space-y-6 pt-4 border-t border-neutral-white/5">
                    <div className="flex justify-between items-center">
                      <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                          <label className="text-[10px] font-bold text-neutral-white/30 uppercase tracking-widest">Fatuística (Técnica / Fato)</label>
                          <button
                            type="button"
                        ...
                        <p className="text-[8px] font-medium text-neutral-white/10 uppercase tracking-widest">Técnica Principal do Experimento</p>
                      </div>
                      <button 
                        type="button"
                        onClick={handleAddFatuistica}
                        className="px-3 py-1 rounded-full bg-pastel-indigo/10 border border-pastel-indigo/20 text-[8px] font-black text-pastel-indigo uppercase tracking-widest hover:bg-pastel-indigo/20 transition-all cursor-pointer"
                      >
                        + Nova Técnica
                      </button>
                    </div>
                    ...
                    <div className="relative group/sel">
                      <select 
                        ...
                        className="w-full p-4 rounded-3xl border border-neutral-white/5 outline-none text-xs font-bold transition-all bg-neutral-black text-neutral-white focus:border-pastel-indigo appearance-none cursor-pointer pr-10 animate-none hover:border-neutral-white/20 select-none text-left"
                      >
                        ...
                        {customFatuistica.map(cf => (
                          <option key={cf} value={cf} className="bg-neutral-black text-white">{cf}</option>
                        ))}
                      </select>
                      <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-white/30 pointer-events-none" />
                    </div>

                    {customFatuistica.length > 0 && (
                      <div className="flex flex-wrap gap-2 pt-1">
                        {customFatuistica.map(cf => (
                          <span key={cf} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-[9px] font-bold bg-pastel-indigo/10 border border-pastel-indigo/15 text-pastel-indigo">
                            <span>{cf}</span>
                            <button
                              type="button"
                              onClick={() => handleEditFatuistica(cf)}
                              className="p-1 hover:bg-neutral-white/10 rounded-full text-neutral-white/40 hover:text-white transition-colors cursor-pointer border-none flex items-center justify-center bg-transparent"
                              title="Editar"
                            >
                            ...
                            <button
                              ...
                              className="p-1 hover:bg-pastel-pink/15 rounded-full text-neutral-white/40 hover:text-pastel-pink transition-colors cursor-pointer border-none flex items-center justify-center bg-transparent"
                              title="Excluir"
                            >
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  ...
                  <div className="p-4 md:p-10 bg-neutral-white/[0.02] border border-neutral-white/5 rounded-[2rem] md:rounded-[4rem] flex flex-col">
                    ...
                    <div className="horizontal-flow-4-rows flex-1">
                      {[...DEFAULT_SENSATIONS.map(s => ({ label: s, isDefault: true })), ...customSensations.map(s => ({ label: s, isDefault: false }))].map(s => (
                        <div key={s.label} className="relative group/tag">
                          <button 
                            type="button"
                            onClick={() => {
                              const prev = task.energyWorkExecution?.sensations || [];
                              const updated = prev.includes(s.label) ? prev.filter(t => t !== s.label) : [...prev, s.label];
                              onUpdate({
                                ...task,
                                energyWorkExecution: {
                                  ...task.energyWorkExecution || { intensity: 0, technique: 'ev', holosomaticImpacts: { physical: 0, energy: 0, emotional: 0, mental: 0 }, symmetry: 1, signals: '', sensations: [], lucidity: 1, phenomena: [] },
                            ...
                            className={`px-4 py-3 md:px-6 md:py-4 rounded-2xl md:rounded-3xl text-[11px] md:text-[11px] font-bold uppercase transition-all flex items-center gap-2 md:gap-3 w-full sm:w-auto justify-center sm:justify-start ${
                              (task.energyWorkExecution?.sensations || []).includes(s.label)
                          ...
                          >
                            <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${ (task.energyWorkExecution?.sensations || []).includes(s.label) ? 'bg-neutral-black' : 'bg-pastel-indigo' }`} />
                            {s.label}
                          </button>
                          {isGlobalEditActive && !s.isDefault && (
                            <div className="absolute -top-2.5 -right-2 flex items-center gap-0.5 transition-all z-20">
                              <button
                                type="button"
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                className="p-1 bg-pastel-indigo text-neutral-black hover:bg-white rounded-full shadow-md flex items-center justify-center cursor-pointer border-none"
                                title="Editar"
                              >
                                <Edit2 className="w-2.5 h-2.5" />
                              </button>
                              <button
                                ...
                                className="p-1 bg-pastel-pink text-neutral-black hover:bg-white rounded-full shadow-md flex items-center justify-center cursor-pointer border-none"
                                title="Excluir"
                              >
                                <X className="w-2.5 h-2.5" />
                              </button>
                  ...
                  <div className="p-4 md:p-10 bg-neutral-white/[0.02] border border-neutral-white/5 rounded-[2rem] md:rounded-[4rem] flex flex-col">
                    ...
                    <div className="horizontal-flow-4-rows flex-1">
                      {[...DEFAULT_PHENOMENA.map(p => ({ label: p, isDefault: true })), ...customPhenomena.map(p => ({ label: p, isDefault: false }))].map(p => (
                        <div key={p.label} className="relative group/tag">
                          <button 
                            type="button"
                            onClick={() => {
                              const prev = task.energyWorkExecution?.phenomena || [];
                              const updated = prev.includes(p.label) ? prev.filter(t => t !== p.label) : [...prev, p.label];
                              onUpdate({
                                ...task,
                                energyWorkExecution: {
                                  ...task.energyWorkExecution || { intensity: 0, technique: 'ev', holosomaticImpacts: { physical: 0, energy: 0, emotional: 0, mental: 0 }, symmetry: 1, signals: '', sensations: [], lucidity: 1, phenomena: [] },
                            ...
                            className={`px-4 py-3 md:px-8 md:py-4 rounded-2xl md:rounded-3xl text-[11px] md:text-[11px] font-bold uppercase transition-all flex items-center gap-2 md:gap-3 w-full sm:w-auto justify-center sm:justify-start ${
                              (task.energyWorkExecution?.phenomena || []).includes(p.label)
                          ...
                          >
                            <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${ (task.energyWorkExecution?.phenomena || []).includes(p.label) ? 'bg-neutral-black' : 'bg-pastel-purple' }`} />
                            {p.label}
                          </button>
                          {isGlobalEditActive && !p.isDefault && (
                            <div className="absolute -top-2.5 -right-2 flex items-center gap-0.5 transition-all z-20">
                              <button
                                type="button"
                                onClick={(e) => {
                                ...
                                className="p-1 bg-pastel-purple text-neutral-black hover:bg-white rounded-full shadow-md flex items-center justify-center cursor-pointer border-none"
                                title="Editar"
                              >
                                <Edit2 className="w-2.5 h-2.5" />
                              </button>
    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          className="relative w-full max-w-full md:max-w-[85%] h-full md:h-auto md:max-h-[90vh] bg-neutral-black md:border border-neutral-white/10 rounded-none md:rounded-[4rem] flex flex-col shadow-[0_20px_50px_rgba(0,0,0,0.35)] dark:shadow-[0_0_150px_rgba(0,0,0,0.8)] overflow-y-auto overflow-x-hidden md:overflow-hidden backdrop-blur-3xl"
        >
          ...
          {/* Header Experience */}
          <header className="px-5 md:px-12 py-4 md:py-5 border-b border-neutral-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4 relative md:sticky top-0 overflow-hidden bg-neutral-black z-30 flex-shrink-0">
          ...
          <div className="flex-1 md:overflow-y-auto overflow-x-hidden p-5 md:p-12 relative bg-neutral-black custom-scrollbar">
          ...
          {/* Footer Navigation */}
          <footer className="px-5 md:px-12 py-4 md:py-5 border-t border-neutral-white/5 bg-neutral-black flex flex-col md:flex-row items-center justify-between gap-6 relative md:sticky bottom-0 z-30 flex-shrink-0 mt-auto">
